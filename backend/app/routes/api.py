...
from app.auth import require_operator, require_user
...
@router.post("/api/complaints", response_model=Complaint, status_code=201, dependencies=[Depends(require_user)],
             responses={400: {"model": ValidationErrorResponse}, 429: {"model": ErrorResponse}})
def create_complaint(data: ComplaintCreate, request: Request, response: Response, service: Service):
    try:
        return service.create(data, client_ip(request), request.state.request_id)
...
@router.get("/api/stats", response_model=Stats, dependencies=[Depends(require_user)], responses={200: {"headers": {"X-Cache": {"description": "Redis cache state", "schema": {"type": "string", "enum": ["HIT", "MISS"]}}}}})
